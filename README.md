...
| **Fly.io Corrosion**<br/>`superfly/corrosion`<br/>*(Distributed SQLite cluster)* | [**PR #559**](https://github.com/superfly/corrosion/pull/559)<br/>[**PR #578**](https://github.com/superfly/corrosion/pull/578) | **PostgreSQL wire protocol & schema crash resilience**: Implemented transaction abort state machine (SQLSTATE `25P02`) and Extended Query sync. In PR #578, eliminated panic during schema migrations with graceful error propagation across agent nodes. | ![PR Open](https://img.shields.io/badge/PR-Open_•_In_Review-blue?style=flat-square) |
...
| **Fly.io Macaroon**<br/>`superfly/macaroon`<br/>*(Auth & discharge tokens)* | [**PR #48**](https://github.com/superfly/macaroon/pull/48) | **Third-party polling deadline backoff stability**: Relaxed strict upper bound boundary check in `TestBackoffBeforeDeadline` to prevent flaky failures under fast context evaluations where remaining time equals the full timeout. | ![PR Open](https://img.shields.io/badge/PR-Open_•_In_Review-blue?style=flat-square) |
